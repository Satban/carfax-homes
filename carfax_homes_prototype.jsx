...
// ==============================
// Carfax for Homes — MVP (50-home version)
// - 50-home dataset
// - Filters (min score, beds, baths, year range)
// - Scoring weight editor (auto-normalized)
// - Per-home printable Report (HTML) + JSON export
// - Bulk export CSV/JSON for visible results
// - Close (×) button on report modal
// ==============================
...
const computeHealthScore = (home, weights) => {
  ...
  Object.keys(w).forEach((k) => {
    ...
    const sub = typeof y === "number" ? ageToScore(y) : 60; // unknown = neutral-ish
    total += sub * w[k];
    sum += w[k];
  });
...
const buildHomes = (n) => {
  ...
  for (let i = 1; i <= n; i++) {
    ...
    const yearBuilt = 1975 + (i % 45); // 1975–2019
    const sqFt = 1200 + ((i * 37) % 2500); // 1200–3699
    const beds = 2 + (i % 4); // 2–5
    const baths = i % 2 === 0 ? 2.5 : 2 + (i % 3); // 2–3.5
    ...
    const soldYear = 2016 + (i % 8); // 2016–2023
...
const ScoreBadge = ({ score }) => {
  const color = score >= 85 ? "bg-green-600" : score >= 70 ? "bg-yellow-500" : "bg-red-600";
  return <span className={`inline-flex items-center px-2 py-1 text-white rounded-full text-xs font-semibold ${color}`}>Health {score}/100</span>;
};
const AgeBar = ({ label, year }) => {
  ...
  return (
    <div className="mb-2">
      ...
      <div className="h-2 bg-gray-200 rounded"><div className="h-2 rounded bg-indigo-500" style={{ width: `${pct}%` }} /></div>
      <div className="text-[10px] text-gray-500 mt-1">{typeof age === "number" ? `${age} yrs old` : "age unknown"}</div>
    </div>
  );
};
const Section = ({ title, children }) => (
  <div className="mt-6"><h3 className="text-lg font-semibold text-indigo-900">{title}</h3><div className="mt-2">{children}</div></div>
);
  ...
  return (
    <motion.div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      <motion.div className="bg-white rounded-2xl w-full max-w-4xl max-h-[92vh] overflow-y-auto shadow-2xl" initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 30, opacity: 0 }}>
        <div className="p-6 bg-gradient-to-r from-indigo-600 via-violet-600 to-fuchsia-600 text-white rounded-t-2xl relative">
          <div className="text-xs uppercase tracking-wide opacity-90">Carfax for Homes — Report</div>
          <button onClick={onClose} aria-label="Close report" className="absolute top-4 right-4 text-white text-xl font-bold leading-none">×</button>
            ...
            <Card className="col-span-2"><CardContent className="p-4">
                  ...
                  <div>
                    ...
                    {(home.permits?.length ? home.permits : [{ date: "—", type: "None recorded" }]).map((p, idx) => (
                      <div key={`${p.id || p.type}-${idx}`} className="flex items-start justify-between border-b last:border-0 py-2">
                        <div><div className="font-medium">{p.type}</div><div className="text-xs text-gray-500">{p.contractor || "—"}</div></div>
                  ...
                  <div>
                    ...
                    {(home.maintenance?.length ? home.maintenance : [{ date: "—", item: "None recorded" }]).map((m, idx) => (
                      <div key={`${m.item || "item"}-${idx}`} className="flex items-start justify-between border-b last:border-0 py-2">
                        ...
                        <div className="text-xs text-gray-500">{m.date}{m.provider ? ` · ${m.provider}` : ""}</div>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <Section title="Disclosures & Notes">
                <div className="space-y-1 text-sm">
                  ...
                  {home.notes && <div className="text-xs text-gray-500 mt-2">{home.notes}</div>}
                </div>
              </Section>
            </CardContent></Card>
...
// ---------- Main App ----------
export default function CarfaxForHomesApp() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-indigo-50">
      <header className="p-6 bg-gradient-to-r from-indigo-600 via-violet-600 to-fuchsia-600 text-white">
        <div className="max-w-6xl mx-auto">
          <div className="text-xs uppercase tracking-wide opacity-90">MVP Preview</div>
          <h1 className="text-3xl font-bold leading-tight">Carfax for Homes</h1>
          <p className="text-white/80 mt-1">Search, filter, open a detailed report, export and share.</p>

          <div className="mt-4 grid md:grid-cols-3 gap-3 max-w-5xl">
          ...
          <div className="mt-4 bg-white/10 rounded-xl p-3 backdrop-blur">
            <div className="text-xs font-semibold mb-2">Scoring Weights (auto-normalized)</div>
            <div className="grid sm:grid-cols-5 gap-2 text-xs">
              {Object.keys(DEFAULT_WEIGHTS).map((k) => (
                <div key={k} className="flex items-center gap-2">
                  ...
                  <span className="w-10 text-right">{Math.round((weights[k] / (Object.values(weights).reduce((a,b)=>a+b,0)||1)) * 100)}%</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <main className="p-6 max-w-6xl mx-auto">
        <div className="text-xs text-gray-500 mb-2">Showing {filtered.length} of {homes.length} homes</div>
        <div className="grid lg:grid-cols-3 gap-4">
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          <div className="text-xs text-gray-500">Built {home.yearBuilt} • {(home.sqFt && home.sqFt.toLocaleString ? home.sqFt.toLocaleString() : home.sqFt)} sq ft</div>
                      ...
                      <div className="grid grid-cols-2 gap-3 mt-4">
                        <div>
                          <div className="text-xs text-gray-500 mb-1">Systems Snapshot</div>
                        ...
                        <div>
                          <div className="text-xs text-gray-500 mb-1">Last Sold</div>
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        ...
                        <Button size="sm" variant="secondary" onClick={() => navigator.clipboard && navigator.clipboard.writeText(((window && window.location && window.location.href) || '') + `#${home.id}`)}>Copy Permalink</Button>
                      </div>
                    </CardContent>
                  </Card>
                </motion.div>
          ...
          <div>
            <Card className="rounded-2xl"><CardContent className="p-4">
              ...
              <div className="text-xs text-gray-500 mb-2">Attach internal notes to the currently selected home.</div>
              <Textarea placeholder="Type a note and save to the selected home…" value={newNote} onChange={(e) => setNewNote(e.target.value)} />
              ...
              <div className="text-xs text-gray-500 mt-3">Tip: Click any card's "Open Report" to select it.</div>
            </CardContent></Card>
          </div>
        </div>
      </main>
      ...
      <footer className="p-6 text-center text-xs text-gray-500">© {new Date().getFullYear()} Carfax for Homes — Demo MVP</footer>
    </div>
  );
}
